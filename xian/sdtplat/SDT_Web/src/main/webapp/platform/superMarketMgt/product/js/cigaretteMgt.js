 ...
 */
    ...
    GridModel = function() {
        return {
            ...
            show: function(){
                ...
                var commands=["create","create","create","retrieve","query"];
    ...
    Ext.onReady(function(){
    	...
    	var filter = function(record, id){
		   if (record.get("FLMC")=='香烟' || record.get("FLMC")=='水果')
		      return true;
		   else
		      return false;
		};
    	...
    	ProductInfoStore.on('load', function (s, records){  
            //s.filter('FLMC','香烟', true, false);
           // s.filter('FLMC','水果', true, false);
    		s.filterBy(filter);
        });
    	PrisonInfoStore.on('load', function (s, records){  
            s.filter('LXFS','开', true, false);
        });
