...
import org.sdt.module.security.service.password.PasswordStrategyExecuter;
...
public class UserService {
	private static final SDTLogger LOG = SDTLoggerFactory
			.getSDTLogger(UserService.class);
	 ...
	 * @return
	 */
	public Page<User> getOnlineUsers(int start, int len, String orgIdStr,
			String roleIdStr) {
		int orgId = -1;
		int roleId = -1;
	...
	public Map<String, Object> modifyPassword(String oldPassword,
		...
		String oldPassword2 = passwordEncoder.encodeold(oldPassword.trim(), user);
		if (oldPassword.equals(user.getPassword())||oldPassword2.equals(user.getPassword())) {
	...
	public PropertyCriteria buildPropertyCriteria(
			PropertyCriteria propertyCriteria, int orgId) {
	...
	public void checkModel(User model) {
		...
		List<User> list = serviceFacade.query(User.class, null,
	...
	public void assemblyModelForPartUpdate(List<Property> properties, User user) {
		for (Property property : properties) {
			if ("password".equals(property.getName().trim())) {
				// 先对用户的密码策略进行验证
				try {
					passwordStrategyExecuter.check(property.getValue()
							.toString());
				} catch (PasswordInvalidException e) {
	 ...
	 *            模型
	 * @param roles
	 *            角色，形如：role-1,role-2,role-3
	 * @param positions
	 *            岗位，形如：position-1,position-2,position-3
	 * @param userGroups
	 *            用户组，形如：userGrou-1,userGrou-2,userGrou-3
	 */
	public void assemblyModelForUpdate(User model, String roles,
			String positions, String userGroups) {
	 ...
	 *            用户组，形如：userGrou-1,userGrou-2,userGrou-3
	 */
	public void assemblyModelForCreate(User model, String roles,
			String positions, String userGroups) {
		// 先对用户的密码策略进行验证
		try {
		...
		if (StringUtils.isNotBlank(positions)) {
			...
			for (String id : positionIds) {
				...
				if (attr.length == 2) {
					...
					Position temp = serviceFacade.retrieve(Position.class,
							positionId);
					if (temp != null) {
		...
		if (StringUtils.isNotBlank(userGroups)) {
			...
			for (String id : userGroupIds) {
				...
				if (attr.length == 2) {
					...
					UserGroup temp = serviceFacade.retrieve(UserGroup.class,
							userGroupId);
					if (temp != null) {
