...
package org.sdt.platform.compass;
 ...
 * @author kimchy
 */
public class SDTLocalCompassBean implements FactoryBean, InitializingBean, DisposableBean, BeanNameAware, ApplicationContextAware, BeanClassLoaderAware {

    protected static final SDTLogger log = SDTLoggerFactory.getSDTLogger(SDTLocalCompassBean.class);
    ...
    public void setConnection(Resource connection) {
        ...
        try {
            log.info("开始执行sdt对compass的定制修改1 :将索引目录路径【 " + connection.getFile().getPath() + " 】修改成路径【 " + this.connection.getFile().getAbsolutePath() + "】");
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (resourceJarLocations != null && !"".equals(resourceJarLocations.trim())) {
            log.info("开始执行sdt对compass的定制修改2");
        ...
        if (resourceDirectoryLocations != null && !"".equals(resourceDirectoryLocations.trim())) {
            log.info("开始执行sdt对compass的定制修改3");
