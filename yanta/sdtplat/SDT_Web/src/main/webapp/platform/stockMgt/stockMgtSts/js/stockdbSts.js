...
ReportOpt = function() {
	return {
		...
		reportShow : function(param) {
			...
			//搜索条件
			var csmc = Ext.get('csid').dom.value;
    		var hpbm = Ext.getCmp('hpbm').getValue();
    		if (csmc != undefined && csmc != '' && csmc != '请选择'){
    			condtion = condtion + ' and CSMC=\'' + csmc + '\'';
    		}
    		if (hpbm != undefined && hpbm != ''){
    			condtion = condtion + ' and HPBM=\'' + hpbm + '\'';
    		}   		  
    		title = ReportFunc.getTitle(param,'调拨');
        	...
        	condtion = condtion + " and (DBSJ>='"+ beginDate + "' and " + "DBSJ<='"+endDate + "')";
    		...
    		this.report_url = ReportFunc.getReportUrl("stockdbSts.rptdesign",title,infoLeft,infoRight,condtion);
