...
@Controller
@Namespace("/cardMgt")
public class SubsidyApplyAction extends ExtJSSimpleAction<subsidyApply> {
    ...
    protected void renderJsonForRetrieve(Map json) {
        ...
        String sql = "select id,rybh,jqmc,xm,xb,zhzt,ye,ssyf,czje,czbz from cardrechargerecord where czlx='生活补贴' and tdbh=?";
    ...
    public String getApply() {
        ...
        List<subsidyApply> applys = getService().query(subsidyApply.class,
                null, propertyCriteria).getModels();

        Map json = new HashMap();
        if (applys.size() > 0) {
            json.put("success", false);
            json.put("message", jqmc + "[" + ssyf + "]生活补贴已发");
    ...
    public String getXLSData() {
        ...
        try {
			...
			propertyCriteria.addPropertyEditor(new PropertyEditor("SSYF",
					Operator.eq, ssyf));
			List<subsidyApply> ralist = getService().query(
    ...
    @Override
    public void beforePartUpdateModel(subsidyApply t) {
        try {
