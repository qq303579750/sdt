...
var authorityAction = 'subsidy-mgt';
    ...
    return {
        getItems: function() {
             var items = [
                      {
                      	  cls:'title',
                    	  items:[{xtype:'label',html:'生活补贴批量导入'}]
					  },
					  {
                    	  xtype: 'panel',
                          layout: 'fit',
				...
				buttons: [
			                {
			                    ...
			                    handler: function() {
									...
									importExcel.Panel.form.submit({
										waitTitle : '请稍等',
										waitMsg : '正在导入......',
										url : contextPath+ '/cardMgt/subsidy-apply!importData.action',
	...
	return{
		sendRequest: function(fileName,ssyf,jqmc){
   			parent.Ext.Ajax.request({
				waitTitle : '请稍等',
				waitMsg : '正在导入......',
				url : contextPath+ '/cardMgt/subsidy-apply!getXLSData.action',
				method : 'POST',
				params:{   
				...
				success : function(form, action) {
					...
					if(data.root.length==0){
						...
						if(!data.success){
			                ...
			                CreateBaseModel.show('批量导入', 'purchaseOrder', 800, 500, CreateOrder.getItems(),data);
			                ...
			                CreateBaseModel.show('生活补贴导入', 'purchaseOrder', 1000, 550, CreateInfo.getItems(),data);
	                    	...
	                    	for(var i=0;i<data.root.length;i++){
   			                ...
   			                //parent.Ext.getCmp('T_ZJRS').setValue("69人");
...
GridModel = function() {
    return {
        ...
        show: function(){
            ...
            var tips=['补贴发放','批量导入','重新提单','详细(D)','高级搜索(S)','显示全部(A)','导出(E)'];
            var callbacks=[GrantModel.show,GridModel.importData,SubsidyModify.show,GridBaseModel.detail,GridBaseModel.advancedsearch,GridBaseModel.showall,GridBaseModel.exportData];
