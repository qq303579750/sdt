	...
	n = tab
			.add({
				...
				html : '<iframe id="'
						...
						+ "windows"
						+ id
						+ '" scrolling="auto" frameborder="0" width="100%" height="100%" onload="Ext.Msg.hide()" src="'
						+ url + '"></iframe>'
			});
	tab.setActiveTab(n);
	id++;
					...
					"beforetabchange" : function(TabPanel, newTab, currentTab) {
						if (newTab != undefined) {
							...
							if (subWindow != undefined
									&& subWindow.GridBaseModel != undefined
									&& subWindow.GridBaseModel.grid != undefined) {
...
function createViewport() {
	...
	Ext.Ajax.request({
		url : contextPath
				+ '/module/module!query.action?node=root&recursion=false',
			...
			for (var i = 0; i < arr.length; i++) {
				var panel = new Ext.tree.TreePanel({
					...
					loader : new Ext.tree.TreeLoader({
						dataUrl : contextPath
								+ '/module/module!query.action?recursion=false'
					}),
				...
				panel.on('expand', function(p) {
					Ext.util.Cookies.set('activedPanelId', p.id, new Date(
							new Date().getTime() + (1000 * 60 * 60 * 24 * 7)),
							contextPath);
				});
				// 激活上次点击的panel
				// if (arr[i].id == activedPanelId) {
				// westPanel.layout.activeItem = tempPanal;
				// }
			}
			westPanel.layout.activeItem = tempPanal;
