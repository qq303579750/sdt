    ...
    return {
				...
				success : function(form, action) {
					...
					if(result.root.length==0){
						...
						CreateBaseModel.shouldSubmit=function(){
			       		    var URL=contextPath+'/cardMgt/card-recharge-record!createSubsidy.action';
			       			parent.Ext.Ajax.request({
		                ...
		                CreateBaseModel.show('生活补贴发放', 'purchaseOrder', 1000, 550, SubsidyGridModel.getItems(),result);
		                ...
		                parent.Ext.getCmp('ZRS').setValue(result.root.length);
		                parent.Ext.getCmp('RS').setValue(result.root.length+"人");
        ...
        search: function(data,alias){
        	if(data[0]=="" || data[1]=="" || data[2]==""){
        		...
        		//SubsidyGrid.silentSearch(data,alias)
        		
        		parent.Ext.Ajax.request({
    				waitTitle : '请稍等',
    				waitMsg : '正在导入......',
    				url : contextPath+ '/cardMgt/subsidy-apply!getApply.action',
    				method : 'POST',
    				params:{   
...
SubsidyGridModel = function() {
    return {
        getItems: function() {
             var items = [
                      {
                      	  cls:'title',
                    	  items:[{xtype:'label',html:'生活补贴发放'}]
					  },
					  {
                          layout: 'column',
                          defaults: {
...
SubsidySetPrisonModel = function(){
	return {
		getItems : function(){
            var items=[
                      {
        ...
        callback : function(){               
            ...
            SubsidyGrid.search(data, "PrisonInfo");
    },
    
    show: function() {
    	SubsidyGrid.getLabelWidth=function(){
            return 100;
        };
        SubsidyGrid.show('选择监区','personInfo', 400, 200, this.getItems(), this.callback);
    }
	};
}();

SubsidyModify=function(){
	return{
		show:function(){
   				...
   				parent.Ext.Ajax.request({
   					...
   					success : function(form, action) {
   						...
   						if(result.root.length==0){
   							...
   							CreateBaseModel.grid = GridRewardInModelInForm.getGrid(true);
   							CreateBaseModel.shouldSubmit=function(){
   				       		    var URL=contextPath+'/cardMgt/card-recharge-record!updataSubsidy.action';
   				       			parent.Ext.Ajax.request({
   				                    url : URL+'?time='+new Date().toString(),
   				                    waitTitle: '请稍等',
   				                    waitMsg: '正在提交充值申请……',
   			                ...
   			                CreateBaseModel.show('生活补贴发放', 'purchaseOrder', 1000, 550, CreateInfo.getItems(),result);
