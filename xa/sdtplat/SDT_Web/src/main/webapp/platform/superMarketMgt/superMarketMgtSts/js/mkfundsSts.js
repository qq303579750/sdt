...
ReportOpt = function() {
	return {
		...
		reportShow : function(param) {
			...
			//搜索条件
    		var csid = Ext.get('csid').dom.value;
    		var hpbm = Ext.getCmp('hpbm').getValue();
    		if (csid != undefined && csid != ''&&csid != '请选择'){
    			condtion = condtion + ' and XSZD_NAME=\'' + csid + '\'';
    		}
    		if (hpbm != undefined && hpbm != ''){
    			condtion = condtion + ' and HPBM=\'' + hpbm + '\'';
    		}   		  
    		title = ReportFunc.getTitle(param,'超市账务');
