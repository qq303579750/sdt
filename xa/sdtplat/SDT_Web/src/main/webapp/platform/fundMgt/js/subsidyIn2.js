 ...
 */
    ...
    var authorityAction ='subsidy-in';   
    	...
    	return{
            verify : function(){
                ...
                if(idList.length<1){
                	...
                	var SSYF    = GridBaseModel.getFieldList("THIS_TIME_SHBT_MONTH");
                	var ssjq    = SHJQ_id[0];
            		var ssyf    = SSYF[0];
...
//	       			}
	       			...
	       			var ssyfList=GridBaseModel.getValueList('THIS_TIME_SHBT_MONTH');
                    ...
                    parent.Ext.MessageBox.show({
                        ...
                        fn: function (id, msg) {
                            ...
                            if(!PubFunc.isYearAndMonth(msg)){
                				...
                				var record = GridBaseModel.grid.getSelectionModel().getSelections();
                	   			for(var i=0; i<record.length; i++){
                					record[i].data['THIS_TIME_SHBT_MONTH'] = msg;
                				}
                	   			GridBaseModel.grid.view.refresh();
                            }
                        },
    	    ...
    	    show : function(idList){
    			...
    			this.window = new Ext.Window({
    				title : '【生活补贴入账】操作员：'+parent.realName+'&nbsp&nbsp&nbsp所属部门：'+parent.orgName,
    	            maximizable:true,
    	            iconCls:'create',
    				width :  400,
    				height : 220,
        ...
        return {
            getFields: function(){
                ...
                {name: 'id'},
 				...
 				{name: 'CZBZ'},
 				{name: 'LAST_TIME_SHBT'},
 				{name: 'LAST_TIME_SHBT_MONTH'},
 				{name: 'THIS_TIME_SHBT_MONTH'},
 				{name: 'RYJG'},
 				{name: 'version'}
			];
               return fields;     
...
//				},
 				...
 				{header: "上次入账时间", width: 23, dataIndex: 'LAST_TIME_SHBT', sortable: true},
 				{header: "上次入账月份", width: 23, dataIndex: 'LAST_TIME_SHBT_MONTH', sortable: true},
 				{header: "本次入账月份", width: 23, dataIndex: 'THIS_TIME_SHBT_MONTH', sortable: true,css:PubCSS.noBlankField(4)},
            ...
            show: function(){
                ...
                var pageSize=15;
                var commands=["create","create","create","retrieve","search","query"];
                var tips=['补贴入账(U)','设置缺省补贴金额','设置补贴所属月份','详细(D)','高级搜索(S)','显示全部(A)'];
                var callbacks=[BatchOpt.verify,BatchOpt.defaultFuntIn,BatchOpt.defaultMonth,GridBaseModel.detail,GridBaseModel.advancedsearch,GridBaseModel.showall];
